...
export default class AuthController {
  public async login({ request, response, auth }: HttpContextContract) {
    ...
    if (user.status !== 0) {
      return response.badRequest({
        msg: `acount status in ${status[user.status]}`,
      });
    }
    if (user.writer !== 0) {
    ...
    try {
      ...
      if (!passwordVerify) {
        return response.badRequest({ msg: "Contraseña Incorreta" });
      }
  ...
  public async authWriter({ request, response, auth }: HttpContextContract) {
    ...
    try {
      ...
      if (user.status !== 0) {
        return response.badRequest({
          msg: `Estado dela cuenta: ${status[user.status]}`,
        });
      }
      const passwordVerify = await user.verifyPassword(password);
      if (!passwordVerify) {
  ...
  public async check({ auth, response }: HttpContextContract) {
    if (!(await auth.use("api").check())) {
      return response.unauthorized({ msg: "unautorized",ok: true});
    }
